...
// Esconde as barras de clima e mensagem ao carregar a página
document.addEventListener('DOMContentLoaded', () => {
    ...
    agora.setSeconds(0);  // Zera os segundos
    agora.setMilliseconds(0);  // Zera os milissegundos

    // Ajusta para o próximo minuto, se já estiver em um novo minuto
    if (agora.getSeconds() === 0) {
        agora.setMinutes(agora.getMinutes() + 1);
    }

    // Formata a data e hora no formato ISO para o campo datetime-local
    const dataHoraAtual = agora.toISOString().slice(0, 16); 

    // Define o valor mínimo para o campo de data e hora
...
document.getElementById('formulario-recomendacao').addEventListener('submit', async (event) => {
    ...
    // Verifica se o raio é um valor válido
    if (raio < 0) {
